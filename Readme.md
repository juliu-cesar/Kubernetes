...
<img src="https://github.com/juliu-cesar/Portfolio/assets/121033909/fe0dd15b-5eee-4cc1-993f-4f7394bcf795" height="400"/>

</div>

Desenvolvido pela Google, o Kubernetes é uma plataforma de orquestração de contêineres de código aberto que visa simplificar a gestão e operação de aplicativos em contêineres. Ela automatiza a implantação, o dimensionamento e a gestão de aplicativos em uma plataforma poderosa para desenvolvedores e administradores de sistemas. Temos alguns pontos a destacar como a **arquitetura baseada em cluster**, a **escalabilidade** a  **automação**, a **compatibilidade com Multi-Cloud**.
...
> Apesar do Kind não precisar do [KubeCtl](https://kubernetes.io/docs/tasks/tools/) para rodar, sera necessário utilizar essa ferramenta para se comunicar com o Kubernetes.
...
Para criar um novo cluster basta executar o comando `kind create cluster`, que ao final do processo teremos um container Docker com o Kubernetes rodando e ao final do processo o seguinte comando sera exibido: 

```bash
kubectl cluster-info --context kind-kind
```

Este comando é utilizado para definir o contexto do KubeCtl, e o `kind-kind` é o nome do cluster (é possível altera-lo opcionalmente no comando para criar o cluster). Podemos verificas os nodes em execução com o comando `kubectl get nodes`.
...
Para verificar a quantidade de nodes que estão em execução podemos utilizar o comando visto anteriormente `kubectl get nodes`.
...
Primeiramente criaremos manualmente um Pod para compreender seu funcionamento, mas normalmente não utilizamos esse processo no dia a dia como as ferramentas que veremos nos próximos tópicos (com destaque ao [Deployment](#criando-um-deployment)). O processo de criar um Pod é bastante simples, bastando criar um arquivo declarativo `.yaml` e definir algumas especificações, vejamos o exemplo para o arquivo `kube-go/pod.yaml`:
...
Apos esse processo temos o Pod criado, e para verificar isso basta executar o comando `kubectl get pods` para retornar algumas informações dele.
...
Para criar o ReplicaSet utilizaremos o mesmo comando anterior `kubectl apply -f kube-go/replicaset.yaml`. Com isso o Kubernetes cria os Pods e se encarrega de tentar subir novos caso algum deles caia, mantendo assim a quantidade definida.

### O problema do ReplicaSet

Suponhamos que houve atualizações no programa feito em Golang e uma nova imagem Docker foi criada, então atualizamos o campo `image` do ReplicaSet para a nova versão e efetuamos o apply do arquivo. Porem o que o Kubernetes faz é aplicar a atualização **somente** nos novos Pods criados, onde todos os outros que continuam rodando mantém a versão antiga. Ou seja, seria necessário excluir todos os Pods manualmente para que os novos fossem criados com a atualização. Vejamos como resolver esse problema no tópico seguinte.
...
Bastando mudar o tipo de objeto no campo *kind* e aplicando essas mudanças com o mesmo comando visto anteriormente `kubectl apply`, teremos o Deployment criado. Caso seja feita alguma atualização ou algo que mude o estado dos Pods, o deployment se encarrega de **gradualmente** substituir os Pods antigos pelos novos. Apesar de simples, esta ferramente é bastante util e sera utilizada diversas vezes. 

## Rollout e Revisões

Algo muito interessante ao utilizar o Deployment, é que quando é feito a atualização/alteração no estado dos Pods, além dele criar um novo ReplicaSet ele manterá o anterior. Com isso permitindo que seja feito o processo chamado de **Rollout**, onde sera retornado o estado dos Pods para uma versão anterior. Abaixo temos dois comandos, o primeiro retorna apenas para a versão anterior e o segundo para uma versão de escolha (podendo até mesmo ser uma versão mais nova):
...
Para saber o numero das revisões disponíveis, temos o comando `kubectl rollout history deployment nome_do_deployment`.
...
- Balanceamento de Carga: O Service ja possui por padrão um balanceamento de carga, onde ao receber uma requisição ele redireciona para o Pod numero um, então para o numero dois, para o tres e assim sucessivamente.
- Tipos de Services: Existem diferentes tipos de Services no Kubernetes, incluindo:
  ...
  - NodePort: Expõe o Service em uma determinada porta que pode ser acessada utilizando o IP de um node. Ou seja, independente de que Node seja utilizado, ao passar a porta definida sera redirecionado para o Service.
...
- Seleção de Pods: Os Services usam etiquetas (labels) para selecionar os Pods aos quais devem direcionar o tráfego. Eles podem direcionar apenas para um conjunto específico de Pods com base nas tags associadas a esses Pods.

## Criando um Service

Antes de criar o Service é preciso salientar que ele não sobe os Pods, sendo necessário criar um *ReplicaSet* ou um *Deployment* primeiro. Dito isso, dentro da pasta `kube-go` vamos adicionar o arquivo `service.yaml`:
...
> O comando para aplicar o service segue o mesmo `kubectl apply`. 

Mesmo apos criar o Service não é possível acessa-lo, e isso ocorre por que o Kubernetes disponibiliza a porta apenas para a rede interna. Uma das possibilidades é fazer um redirecionamento de porta, vejamos abaixo:
...
Com isso ao acessar o serviço pela porta 80, ele ira redirecionar para o container na porta 9000. No arquivo do tópico anterior não foi necessário esse passo pois ambas as portas eram iguais (80).
...
Dessa forma é possível acessar o serviço utilizando qualquer um dos Nodes passando a porta `30001`, onde ele ira por sua vez redirecionar para o container.

**Não é muito recomendado utilizar o NodePort**, principalmente por questões de segurança, além de que o proximo tipo acaba sendo mais util.

### LoadBalancer

O LoadBalancer é um dos tipos de Service que permite expor um aplicativo para o tráfego externo. Geralmente utilizado junto a um provedor em nuvem, onde o Kubernetes interage com o provedor para que ele disponibilize um balanceador de carga que terá um IP externo, permitindo que o tráfego alcance o Service de fora do cluster. Assim como os outros tipos basta mudar o objeto no campo `type`:
...
Temos algumas formas para configurar variáveis de ambiente no container. Vejamos algumas
...
Utilizamos o `valueFrom` para indicar que o valor virá de outro arquivo, e `configMapKeyRef` para informar qual o nome do Map que estamos referenciando e o campo com o valor definido. Em seguida precisamos aplicar o Map e atualizar o Deployment.
